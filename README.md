...
- an UART cable with Micro USB termination (3.3V) or make one yourself with a Micro USB cable and an UART USB adapter (must be set to 3.3V, only plug VCC, GND, RXD and TXD, connections must be Key RXD=>Device TXD and vice versa).
- a TFTP server to host the OpenWRT firmware and the binary to bypass firmware protection
